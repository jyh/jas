...
pub(crate) fn dispatch_action(action: &str, params: &serde_json::Map<String, serde_json::Value>, st: &mut crate::workspace::app_state::AppState) -> Vec<serde_json::Value> {
    ...
    // Native intercept: the Symbols and Concepts panels' native verbs
    // (SYMBOLS.md §7-8, CONCEPTS.md §6-10). Their YAML actions are `log`
    // stubs, so they run here, through the ONE implementation the engine's
    // host also calls (W2b-17), and return without further YAML effects.
    // `concepts_panel_select` is NOT one of them: it must ALSO run its generic
    // `set_panel_state` effect, and is tracked natively by the intercept above.
    if crate::interpreter::symbols_host::ACTIONS.contains(&action) {
        use crate::interpreter::symbols_host::{run, Outcome};
        let (sym, con) = (st.symbols_selected.clone(), st.concepts_selected.clone());
        let Some(tab) = st.tab_mut() else { return Vec::new(); };
        match run(&mut tab.model, action, &serde_json::Value::Object(params.clone()),
                  sym.as_deref(), con.as_deref()) {
            Some(Outcome::SelectSymbol(id)) => st.symbols_selected = id,
            Some(Outcome::Dialog { id, params }) => {
                return vec![serde_json::json!({"open_dialog": {"id": id, "params": params}})];
            }
...
pub(crate) fn build_active_document_view(
    ...
    let selected_concept =
        crate::interpreter::document_views::selected_concept_view(tab.model.document());
    ...
    let symbols_json = crate::interpreter::document_views::symbols_view(&doc);
...
fn run_yaml_effect(
    ...
    //
    // The seven `doc.*` artboard keys live in `artboard_effects` (W2b-14),
    // web-free, so the engine hosts them too. They reach it through the
    // `doc.*` fallback at the end of this function.
...
// create path resolves each YAML expr to a JSON literal in `artboard_effects`, journals
...
fn render_panel(el: &serde_json::Value, ctx: &serde_json::Value, rctx: &RenderCtx) -> Element {
    if let Some(content) = el.get("content") {
        ...
        let panel_kind = el.get("id").and_then(|v| v.as_str()).and_then(|id| match id {
            ...
            "brushes_panel_content"    => Some(PanelKind::Brushes),
