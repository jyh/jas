...
mod tests {
    ...
    // Gated like its siblings `workspace_panel_ops` / `workspace_pane_ops`:
    // the DOCK MODEL (WorkspaceLayout, PanelKind) is a web-side module, so a
    // native core build has no layout to ask. The predicate itself is pure
    // data logic and drives no frontend.
    #[cfg(feature = "web")]
    #[test]
