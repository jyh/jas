...
/// Push a `PolygonSet` through an affine matrix, vertex by vertex.
///
...
/// document by `common.transform`. Comparing them raw makes the merge match
/// on where the artwork is STORED rather than where it is DRAWN.
...
/// See BLOB_BRUSH_TOOL.md §Transform.
pub fn transform_polygon_set(
    ps: &[Vec<(f64, f64)>],
    t: &crate::geometry::element::Transform,
) -> Vec<Vec<(f64, f64)>> {
    ps.iter()
        .map(|ring| ring.iter().map(|&(x, y)| t.apply_point(x, y)).collect())
        .collect()
}
...
mod tests {
    ...
    fn closest_on_cubic_point_above_middle_does_not_overflow() {
        ...
        // The `t` assertion is what catches the first three: its 1e-4 bound is
        // 200x tighter than the smallest of their t deviations (0.0199939854).
        // The fourth is caught by `is_finite`. Do not lean on the distance
        // bound alone — for the `d1`/`d2` mutations the relative distance error
        // is only 1.9986e-4, barely past 1e-4.
        let (d, t) = closest_on_cubic(
