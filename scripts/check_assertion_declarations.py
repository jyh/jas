...
⛔ Read this as a boundary, not as an omission. Each line is a decision.
  ...
  * CLAUSES NAMED BY A FUNCTION IN `harness_common.ps1`. Q6's seven (W2-6)
    are built by `Get-SbPaneVerdicts`, and V1-V6 (W2b-3) by
    `Get-SbValueVerdicts`; each family is recorded by a COLUMN-0 loop here,
    so their names never appear in this file and the count above does not
    include them (53 before and after each landed, measured both times). The
    property this gate enforces -- every clause on every path -- is enforced
    for them by `harness_selftest.ps1`'s census cases (nine variants for Q6,
    eleven for V), which is where a function's branches can be driven at all.
