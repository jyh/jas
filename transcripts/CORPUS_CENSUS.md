...
| **48** | `offset_path` variable-width profiles | `algorithms/offset_path.rs` | `Canvas/OffsetPath.swift` | **GATED** (`offset_path`, 8 vectors) | R1 R4 | Stroke panel width profile | **[CLOSED 2026-08-01]** the geometry is split from the rasterisation, so the rails and the caps are VALUES and the family is on the algorithm wire in both ports. **The [SUSPECT] cap note was settled BY MEASUREMENT and the answer is: no divergence.** `OffsetPathCapTests.swift` reads points back out of a real `CGMutablePath` and finds `clockwise: true` to be the decreasing-angle sweep, which is what WHATWG gives canvas's `anticlockwise: true` for the same arguments; the y-flip maps rails and cap through one transform. Moot in any case now, because both ports flatten the cap through shared arithmetic and neither passes a sweep flag to a platform. **WHAT THE EYEBALL WOULD HAVE MISSED, and the reason this row is worth re-reading:** the cap's base angle was `atan2(n_y, -n_x)` in BOTH ports, the arguments reversed, which is the direction REFLECTED rather than the direction -- so every round cap was welded on at the wrong angle except at 135 and 315 degrees. Measured 7.07pt of chord on a 10pt stroke. Fixed in both ports; the three round-cap vectors were red on the oracle while all eight port-vs-port comparisons were green |
