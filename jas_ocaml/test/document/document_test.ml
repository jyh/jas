...
let () =
  ...
  run_test "layers accessible" (fun () ->
  ...
  run_test "default selection is empty" (fun () ->
    assert (PathMap.is_empty doc.selection));

  run_test "selection with paths" (fun () ->
  ...
  run_test "get_element: layer" (fun () ->
    let elem = get_element doc [0] in
    assert (elem = layer0));

  run_test "get_element: child" (fun () ->
    let elem = get_element doc [0; 1] in
    assert (elem = circle));

  run_test "get_element: nested" (fun () ->
    let elem = get_element doc [0; 2; 0] in
    assert (elem = line));

  run_test "get_element: empty path raises" (fun () ->
    (try
       let _ = get_element doc [] in
       assert false
     with Failure _ -> ()));

  run_test "replace_element: child" (fun () ->
  ...
  run_test "replace_element: nested" (fun () ->
  ...
  run_test "replace_element: empty path raises" (fun () ->
    let new_rect = make_rect 5.0 5.0 20.0 20.0 in
    (try
  ...
  run_test "replace_element: result layer is still a Layer" (fun () ->
    let new_rect = make_rect 5.0 5.0 20.0 20.0 in
