...
let () =
  Printf.printf "Shape recognition tests:\n";

  (* Generator sanity *)
  run "generator_circle_has_expected_radius" (fun () ->
  ...
  run "generator_round_rect_runs" (fun () ->
    let pts = sample_round_rect 0.0 0.0 100.0 60.0 10.0 200 in
    assert (List.length pts > 50));

  run "generator_lemniscate_passes_through_origin_offset" (fun () ->
  ...
  run "jitter_is_deterministic" (fun () ->
  ...
  run "recognize_clean_square_emits_rectangle_with_equal_sides" (fun () ->
  ...
  run "recognize_clean_round_rect" (fun () ->
  ...
  run "recognize_clean_circle" (fun () ->
    let pts = sample_circle 50.0 50.0 30.0 64 in
    match recognize pts cfg with
  ...
  run "recognize_clean_ellipse" (fun () ->
  ...
  run "recognize_clean_arrow_outline" (fun () ->
  ...
  run "recognize_clean_lemniscate_horizontal" (fun () ->
  ...
  run "recognize_clean_lemniscate_vertical" (fun () ->
  ...
  (* Noisy positive ID *)
  run "recognize_noisy_circle" (fun () ->
  ...
  run "recognize_noisy_rectangle" (fun () ->
  ...
  run "recognize_noisy_ellipse" (fun () ->
  ...
  run "recognize_noisy_triangle" (fun () ->
  ...
  run "nearly_closed_polyline_treated_as_closed" (fun () ->
    let clean = sample_rect 0.0 0.0 100.0 60.0 16 in
  ...
  run "clearly_open_polyline_not_rectangle" (fun () ->
    let clean = sample_rect 0.0 0.0 100.0 60.0 16 in
  ...
  run "recognize_path_via_bezier_input" (fun () ->
  ...
  (* Disambiguation *)
  run "square_with_aspect_1_04_is_square" (fun () ->
  ...
  run "rect_with_aspect_1_15_is_not_square" (fun () ->
  ...
  run "lemniscate_off_center_crossing_returns_none" (fun () ->
  ...
  (* Element conversion *)
  run "recognized_to_element_preserves_stroke_and_common" (fun () ->
    let open Jas.Element in
  ...
  run "recognized_to_element_round_rect_sets_rx_ry" (fun () ->
    let open Jas.Element in
  ...
  run "recognized_to_element_arrow_emits_polygon" (fun () ->
    let open Jas.Element in
    let template = Path { d = []; fill = None; stroke = None;
      opacity = 1.0; transform = None; locked = false; visibility = Preview } in
  ...
  (* Scribble tests *)
  run "recognize_clean_zigzag_scribble" (fun () ->
  ...
  run "recognize_noisy_zigzag_scribble" (fun () ->
  ...
  run "straight_line_not_scribble" (fun () ->
  ...
  run "diagonal_line_not_scribble" (fun () ->
    let pts = sample_line (0.0, 0.0) (100.0, 80.0) 64 in
    match recognize pts cfg with
    | Some (Recognized_line _) -> ()
    | _ -> failwith "expected Line");

  run "recognized_to_element_scribble_emits_polyline" (fun () ->
    let open Jas.Element in
    let template = Path { d = []; fill = None; stroke = None;
      opacity = 1.0; transform = None; locked = false; visibility = Preview } in
  ...
  (* recognize_element tests *)
  run "recognize_element_skips_line" (fun () ->
    let open Jas.Element in
  ...
  run "recognize_element_skips_rect" (fun () ->
    let open Jas.Element in
  ...
  run "recognize_element_skips_circle" (fun () ->
    let open Jas.Element in
    let elem = Circle { cx = 50.0; cy = 50.0; r = 30.0;
      fill = None; stroke = None; opacity = 1.0; transform = None; locked = false; visibility = Preview } in
    assert (recognize_element elem cfg = None));

  run "recognize_element_skips_polygon" (fun () ->
    let open Jas.Element in
    let elem = Polygon { points = [(0.0, 0.0); (100.0, 0.0); (50.0, 86.6)];
      fill = None; stroke = None; opacity = 1.0; transform = None; locked = false; visibility = Preview } in
    assert (recognize_element elem cfg = None));

  run "recognize_element_converts_path_circle" (fun () ->
    let open Jas.Element in
    let pts = sample_circle 50.0 50.0 30.0 64 in
  ...
  run "recognize_element_square_returns_square_kind" (fun () ->
    let open Jas.Element in
    let pts = sample_rect 0.0 0.0 80.0 80.0 16 in
    let d = List.mapi (fun i (x, y) ->
      if i = 0 then MoveTo (x, y) else LineTo (x, y)) pts in
