...
(* A minimal 3-group test layout. Decouples behavior tests from the
   ...
   actually verify default-layout shape still call [test_layout ()];
   the rest use this. *)
let test_layout () = {
...
let persistence = [
  Alcotest.test_case "reset_to_default" `Quick (fun () ->
    ...
    assert (l.floating = []);
    assert (l.hidden_panels = []);
    (* Exact group count is a default-layout property not under
       test here; other tests validate the default shape. *)
    assert (Array.length (Option.get (anchored_dock l Right)).groups > 0));
]
