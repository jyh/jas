...
**Status: DRAFT for the roadmap sitting. Not ratified. Do not plan against §3 dates —
there are none.**
...
2. 🟨 **PLACEHOLDER** — the record is silent, and the block is left *empty* for the
   Captain to fill at the sitting.
...
**The practical consequence for the sitting: the concept-pack road, which `VISION.md`
still presents as the next critical-path item, is FINISHED. The lane is further along
than its own north star believes, and the sitting should be spent on what comes after it,
not on it.**
...
Open PRs: **1** — flask's `#96`, the Direct2D whole-op witness pass, in draft.
...
SEE that proves the milestone has started, rather than a percentage.** Ordering is by
dependency, not by date — and §3's ordering is itself a question for the sitting (§5).
...
### 3.1 The gesture and lens layer — *pillar 5 + pillar 4*
...
both active ports.
**Why it is early:** it is the cheapest of the grand items *and* it is a prerequisite for
the shaper (§3.4), which is the vision's flagship unbuilt feature.

### 3.2 The AI operation API and canvas perception — *"Claude Code, but for drawing"*
...
the reason the parity work was never a detour. Tool schemas generated from
`actions.yaml`; perception by structural query **and** visual raster; an agentic loop of
perceive → plan a transaction → **live preview** → self-critique → artist accepts.
The key difference from coding, in the vision's own words: **review happens *before*
commit**, and *"the artist — not a test oracle — is the judge of 'good'"* (`:228`).
**First observable:** an operation proposed by a model, previewed live, accepted by the
artist, and landing in the journal as a named, undoable transaction indistinguishable
from a hand-made one — **with `checkpoint_equivalence` green over it.** That last clause
is the milestone: it is what makes artist primacy (§1.6) machine-checked rather than
promised.
...
🟨 **PLACEHOLDER — for the sitting:** which of these four does he want first? They differ
by an order of magnitude in cost and the record does not rank them.

### 3.4 The shaper — freehand ink + inferred precision
📎 Sourced: `transcripts/AI.md:7-19` in full (see §1.3).
**Never queued, never designed, fully described in the founding notes.** Depends on §3.1
(pressure) and §3.2 (inference).
...
### 3.8 Animation — *keep-ready, deliberately deferred*
...
⚠️ Named here because `transcripts/AI.md:30` lists animation as a **project type he
wants**, while `VISION.md` defers it. That tension is real and the sitting should settle
it (§5).
...
**A named milestone with an empty description block, by design.**

⛔ **FINDING, MEASURED AND STATED PLAINLY: the phrase "Last Supper" appears NOWHERE on any
fleet surface.** I ran the census myself rather than inheriting the helm's: zero hits in
the jas tree (all tracked files), zero across the fleet's brief and bank archives, zero
in the working-notes tree, zero in the fleet's own coordination record. The only occurrences anywhere are inside today's council
order itself and in this document. **The vision for it is oral. It exists only in the
Captain's head and in conversation.**

Therefore this section is deliberately empty. **Nothing here is inferred, and nothing here
should be.** An invented paragraph would read exactly like a recorded one to every future
reader of this file, and it would be indistinguishable from his actual intent forever
after — which is the specific failure this whole document is shaped to avoid.

### 4.1 His description — to be filled at the sitting, in his own words

> 🟨 *[the Captain's sitting fills this]*
>
>
>

### 4.2 The questions the sitting must answer

Written so the sitting is a 45-minute conversation with a shape, not a blank page.
...
### 4.3 Why this milestone matters to the roadmap even while empty
...
(`VISION.md:258`). §3's ordering should probably follow this project's needs. It cannot,
until §4.1 is filled.

---

## 5. WHAT THE ROADMAP SITTING DECIDES

Six questions. Everything else in this document is either sourced or explicitly empty.

1. **§4.1 — the Last Supper, in his words.** The one irreplaceable input. Nothing else in
   the sitting can substitute for it.
2. **The ordering of §3.** The dependency ordering is derived; the *priority* ordering is
   his. §3.0 (Windows) is already ruled first. The live question is what comes after it —
   and my recommendation is below.
3. **§3.8 — the animation tension.** `transcripts/AI.md:30` lists animation as a project
   type he wants; `VISION.md:233-240` defers it. Both are his. Which stands?
4. **§3.3 — which semantic relationship first?** Connectors, mirrored eyes, the posed
   figure, the gear (done). They differ by an order of magnitude in cost.
5. **Does `VISION.md` get repaired now?** Four stale claims (§0). My recommendation:
   **yes, and as a separate PR from this one**, so the roadmap draft is not entangled with
   an edit to the north star. **I have not touched `VISION.md` in this branch.**
6. **Is "grander" a change of direction or a change of altitude?** My reading of
   `VISION.md:91-94` is that it is altitude — the parity work *is* the AI architecture,
   seen once. If he means direction instead, §3 is the wrong list and the sitting should
   say so early.

### My recommendation, since a recommendation is owed rather than a survey

**After the Windows app: §3.1 (gesture/pressure), then §3.2 (the AI operation API).**

Three reasons, and the third is the one I would defend hardest:

- §3.1 is the **cheapest** of the grand items and is a measured, confirmed gap (§0) rather
...
- **The browser lane** (69/69) is **2026-09-02's** number, re-verified on merged main
  then, not re-run today. `main` has moved 5 commits since (flask's `#93`, `#94`).
...
- **`VISION.md:396`** — *"Add the widget/effect parity guard and validator cross-reference
  layer — ⬜ not done."* `scripts/check_widget_kind_dispatch.py` exists and covers
  per-app widget-kind dispatch, which looks like the first half. **I did not determine
  whether it discharges the item**, so I have NOT listed it in §0's stale table. It is a
  fifth candidate, unresolved.
...
*Draft prepared by the jas seat, 2026-09-03, for the roadmap sitting (desk EV). Measured
at `main` `a179efe4`. Nothing in §4 is invented; nothing in §1 is paraphrased.*
