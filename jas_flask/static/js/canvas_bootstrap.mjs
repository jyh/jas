...
function wireCanvasEvents(canvasEl, model) {
  ...
  function payload(type, evt) {
    ...
    // Flask V1 has no canvas pan / zoom transform, so doc-space and
    // screen-space coincide. The native apps pass distinct
    // event.x / event.doc_x through their canvas dispatchers and the
    // tool YAMLs read event.doc_x for shape-creation coords so the
    // new element lands at the right document-space point even with
    // a panned canvas. Mirror those keys here so the same YAML works
    // in Flask without per-app branching.
    return {
      type,
      x, y,
      doc_x: x, doc_y: y,
