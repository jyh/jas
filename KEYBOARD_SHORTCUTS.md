...
| Select only the objects that are completely inside the marquee | Press E while dragging the marquee | Press E while dragging the marquee |
...
| Shortcuts | Windows | macOS |
| ----- | ----- | ----- |
| Constrain a shape’s proportions or orientation to: equal height and width for rectangles, rounded rectangles, ellipses, and grids Increments of 45° for line and arc segments Original orientation for polygons, stars, and flares   | Shift-drag | Shift-drag |
...
| Switch Pencil tool to Smooth tool | Alt (Enable **Alt key toggles to Smooth Tool** from **Pencil Tool Options**) | Option (Enable **Option key toggles to Smooth Tool** from **Pencil Tool Options**) |
...
| Fill across unstroked edges into adjacent faces | Double-click \+ Live Paint Bucket tool | Double-click \+ Live Paint Bucket tool |
...
| Transform pattern (independent of object) when using Selection tool, Scale tool, Reflect tool, or Shear tool | Backquote (\`)-drag | Backquote (\`)-drag |
...
| Apply swatch color to active (or selected) color stop | Alt-click swatch in the Swatches panel | Option-click swatch in the Swatches panel |
