...
// `stk_end_arrowhead_scale`) carry an `event: commit` behavior: when
// `panel.link_arrowhead_scale` is true, committing one scale copies it onto
// the OTHER scale in both scopes. The combo now commits through
// PanelWidgetEvents (the widget event contract), and the arms that drove
// the retired `runInputCommitBehavior` are its arms, over the shipped
// widgets: PanelWidgetEventsTests (the linked and unlinked mirrors, the
// stale render scope, the chain flag, and the sibling reaching the
// selection). What stays here is the gap closure on its own.
