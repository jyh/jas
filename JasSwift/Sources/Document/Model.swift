...
public class Model: ObservableObject {
    ...
    // The six View verbs (zoom in / out / actual size, fit active artboard /
    // all artboards / in window) used to live here as native methods with
    // Swift-side copies of zoom_step 1.2, min / max zoom 0.1 / 64.0 and fit
    // padding 20.0, plus `document.artboards.first` where the workspace says
    // `active_document.current_artboard`, plus a `w > 0` guard that made Fit in
    // Window a no-op on an empty document instead of the spec's 100%-centred.
    // They are DELETED: every user path now dispatches the YAML action through
    // ``ViewActions``, which is what the action corpus gates (RULED 2026-07-27,
    // transcripts/ZOOM_TOOL.md, "Swift's MENU AND KEYBOARD must route through
    // the YAML pipeline"). `viewVerbsHaveNoNativeReimplementation` keeps them
    // from coming back by name.
