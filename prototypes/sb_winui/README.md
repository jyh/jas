...
**The app is DPI-AWARE, and the surface is sized in PHYSICAL PIXELS.**
`app.manifest` declares `PerMonitorV2` (with the `true/pm` fallback for the
pre-1703 key) and the csproj names it in `<ApplicationManifest>`. Without it an
unpackaged WinUI 3 app is DPI-unaware and Windows bitmap-virtualises it: on the
box, a 1904×941 logical client stretched to 2856×1464 physical, `GetDpiForWindow`
answering 96, and every injected gesture landing at exactly two thirds of its
asked point. `Canvas.SizeChanged` gives DIPs, so the swapchain size is now
`DIP × CompositionScaleX/Y` — the same scale the pointer path multiplies by, so
the two halves of the chain cannot disagree — and a composition-scale change
re-derives the surface rather than leaving one sized for the old rasterisation.
The startup receipt carries `dpi-awareness=` and `dpi-for-window=` read from the
OS, so the manifest can be ASSERTED rather than inferred from a ratio: the app's
own `scale=1` was TRUE from inside its virtualised view, which is why nothing in
its log could contradict it.
...
| `SB_SCENE` | which workload runs: `benchmark`, `goldens`, `document`, `retained`, `stall`, `pointer`, `stay`, `abi`, `app`, `selection-marquee` (`Canvas.ApplySceneInner`). `abi` is wave 1's PROBE over the four new ABI bindings and says so on its own `ABI` row; it is deliberately NOT named `app`. **`app` is THE APP ENTRY (W4)** — the materialized menubar, the picker, save, Ctrl+Z/Y and the dirty mark; it is run-and-stay and does NOT complete. An empty value resolves to `benchmark` (`MainWindow.StartFirstLayout`), so every historical invocation keeps its meaning; an unrecognised value is refused by name; and the old spelling `selection` is REFUSED pointing at `selection-marquee` rather than silently running it, because that scene is a control and a receipt must never be ambiguous about which arm produced it | `benchmark` | all | benchmark |
...
| `SB_SIZE` | pin the swapchain at an explicit `WxH` in PHYSICAL pixels, so a copy can be priced at a stated surface (`StartFirstLayout`). It is no longer the ONLY way to reach physical pixels — the surface is now derived as the panel's DIP size × `CompositionScaleX/Y` — but it is still the way to state one, and a run pinned here is comparable to every S-B number already on record. A malformed value refuses loudly rather than falling back to the laid-out size, which would label a run 4K and measure it at 3.6 Mpx. Mutually exclusive with `SB_RESIZE`, and with a composition-scale change: a later resize from either is refused by name | *(unset: the laid-out size, in physical pixels)* | all | benchmark |
| `SB_SQUEEZE` | `1` sets `PreferredMinimumHeight = 1` and squeezes the window to the status row, so the panel's `SizeChanged` fires with height 0 through the real window manager rather than through a probe (`MaybeSqueeze`). It needs an `OverlappedPresenter` and REFUSES by name on any other kind — a squeeze that did not happen must not read as a squeeze that was accepted. **It now writes a row in EVERY outcome.** The request row carries the presenter's own `min-height policy=` read back after the write, and the answer arrives as `SQUEEZE delivered <W>x<H> (requested height <n>; min-height policy=<m>) policy=<the decision>` (`Refuse`, `Defer` or `Accept`) on the next `SizeChanged` — or `delivered NONE` after a bounded 2000 ms if the window manager never delivers one. On the box this knob produced NO row at all, and from the log a squeeze the manager ignored and a zero the policy accepted were indistinguishable | *(unset: no squeeze)* | all | interaction |
...
| `SB_OPEN_PATH` | path the `app` scene opens at startup INSTEAD OF the file picker, driving the SAME code path (`Canvas.ApplyOpen`). ⛔ It exists because the picker is the freeze's stop 2 — WinAppSDK pickers need a window handle and, unpackaged, an initialisation call, and that is unmeasured on the box. When the picker throws, the receipt says `open=PICKER-FAILED` and this knob is how P1 is measured anyway, with the picker recorded NOT RUN. **Never a synthetic receipt wearing `PICKER`.** Falls back to `SB_SVG` when unset | *(none: the app starts empty)* | `app` | interaction |
| `SB_SAVE_PATH` | path the `app` scene's Save writes to WITHOUT opening the save picker, driving the SAME code path (`Canvas.ApplySave`). Same argument as `SB_OPEN_PATH`: the picker is a platform unknown and a harness must be able to measure the save round trip without one. Save As always asks | *(none: Save opens the picker)* | `app` | interaction |
...
| `SB_TRACE_POINTER` | any non-empty value writes ONE receipt row per pointer event — `POINTER-TRACE kind=<PRESS / MOVE / MOVE-DUP / MOVE-IGNORED> id= ts= frame= pos= contact= update= intermediates= move-count=` (`MainWindow.xaml.cs`, `TracePointer`). Read ONCE at construction, so a gesture cannot change its own instrumentation half way through. **It is what IDENTIFIED `move != k`**: the extras carry the SAME `FrameId` AND `Timestamp` AND position as an event already applied, so they are XAML RE-DELIVERING a frame rather than new input, and they multiply with the idle gap after each move (settle 10 ms: none; 800 ms: five). The shell now suppresses a repeated frame and reports `dup-frames=` on its `POINTER` row; this knob is how the finding is re-driven. Off by default because a drag writes one row per event into the file that is also the oracle — and it is safe to leave on during a real sitting only because the title carries the RUN's verdict rather than the last row's (`TitleVerdict`) | *(unset: no trace)* | `pointer`, `retained` | interaction |
...
| `SB_UI_STALL_MS` | milliseconds the XAML thread sleeps, posted to it from the scene. O3's ORACLE-LIVENESS control: `Responding` must read `False`, because an oracle that cannot say `False` says nothing when it says `True`. **Set ALONE it now writes O3's `STALL` row too** — the sleep's own continuation posts a `UiStallDoneCmd`, which arms the same latch the render arm arms, so the post-stall drain writes `STALL render-stall=0ms ui-stall=<n>ms` with `stall-tid` naming the XAML thread and a `UI-STALL DONE` row before it. Before this repair the row came only from the render arm's drain, so this knob alone wrote nothing and the harness timed out at 140 s. With `SB_RENDER_STALL_MS` also set the RENDER arm owns the single row and this one is named on it. Unset it and `SB_RENDER_STALL_MS` together and the `stall` scene refuses. Combining it with `SB_PAINT_ON_UI=1` cannot hang: the paint step's hand-over to the sleeping thread refuses by name after 5 s | *(unset: no UI-thread stall)* | `stall` | interaction |
...
⭐ **ADDRESSED IN THE SHELL, NOT YET MEASURED (the first-run repairs).** Two
things were wrong at once and only one of them was named here. The swapchain
took the DIP size (this row), and the process was DPI-UNAWARE, so
`CompositionScaleX` read **1** and the multiply above was an identity for the
wrong reason — the numbers in this row were taken through a virtualised window.
Both are changed: `app.manifest` declares `PerMonitorV2`, and the surface is
`DIP × CompositionScale`. **Nothing below is a measurement of the repair.**
On a 150 % display the reported surface should move from `1904x941` to
`2856x1464`, the pointer rows' `scale=` from `1` to `1.5`, and
`dpi-for-window=` from `96` to `144`; the physical-4K copy cost becomes
measurable for the first time. Every one of those is the box's to read.
...
### The assertion table — every assertion, and its exact expected reading

Each one prints `PASS` / `FAIL` / `NOT RUN` **by name, followed by the row it
...
| `O1.5 H1 != A-MUT` | **PASS** |
| `O1.6 A != A-MUT` | **PASS** — but `NOT RUN` under `mutation=NONE`: with no gesture `A == A-MUT` is a CONSTRUCTION, and a red there would convict the pointer seam of a run that never drove it. `mutation=REAL` or `mutation=SYNTHETIC` is what licenses asserting it |
| `O1.7 probe colours` | **PASS** only with `-ProbeCapture`; otherwise `NOT RUN` with the three counts printed anyway |

#### O2 — the resize drain, priced against ONE FRAME
...
| `O3.C2 SB_PAINT_ON_UI=1 design-red control` | **PASS iff the tid assertion FAILS**, counted over the PAINTING rows |

#### O4 — real input (`-Scene pointer`, and the gesture inside `retained`)
...
| `O4.C1` / `O4.C2` | the same oracles read through the empty-canvas and the `SB_SYNTH_DRAG` provenances |
| any of the above with counters at 0 | `NOT RUN: hand refused` — UIPI failure is undetectable at the injector, so a zero is never a pass |

**`-Scale` no longer exists.** It was measured to reach nothing: `send_hand.ps1`
has no such parameter and derives the factor from `GetDpiForWindow`, so a run
driven with `-Scale 1.5` produced readings identical to one without it, down to
the digit. The scale is READ — this run's rows, else the log's history, else 1.0
— and every detail says which of the three.

#### O5 — run and stay

| name | expected reading |
|---|---|
| `O5.1 the STAY pid row names this run's process` | **PASS** — two independent identifications agreeing |
| `O5.5 pre-existing instances survive the teardown` | **PASS** — `N of N` |
| `O5.6 a refusal touched nothing` | **NEW.** `-Stop` VALIDATES BEFORE IT ACTS, and a refused `-Stop` prints `ok  : [O5.6] the refusal touched nothing` naming the task that is still registered and the stay pid that is still alive. The first run measured the opposite: both refusals still dropped a live stay's scheduled task. With no task and no record to observe, it is `NOT RUN` rather than a free pass. A SUCCESSFUL `-Stop` on a pid the record does not name leaves the task and the record alone for the same reason |

#### O6 — the three-valued surface policy, in TWO runs
